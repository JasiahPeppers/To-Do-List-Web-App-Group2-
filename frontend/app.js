...
<div class="task-wrapper">
<span class="task-title">
<span class="task-number">New Task</span> ${taskValue}
</span>
...
listItem.querySelector(".task-number").textContent = `${data.id}.`;
...
// store task.id here for later 
listItem.dataset.id = task.id; // this should store the task's id in a data attribute 
...
.then(response => response.json())
.then(data => {
console.log('Task has been deleted:', data); // Task deleted successfully
...
// PUT request to update the edited task dec on srver
...
body: JSON.stringify({
desc: textarea.value, // Update task description
})
})
.then(response => response.json())
.then(data => {
 console.log('Task description updated:', data); // Description updated
})
.catch((error) => {
console.error('Error updating task description:', error); // Handle PUT failure
});
}
}); 
